...
/**
 * Migration unifiée — Schéma complet fusionné SI-GPRH + CARRIÈRE-STAGIAIRE
 * 61 tables, 1 vue, index de performance
 * Base de données PostgreSQL
 */
class CreateUnifiedSchema extends Migration
{
    public function up()
    {
        // Exécuter le schéma SQL complet fusionné
        $sql = <<<'SQL'
...
CREATE TABLE IF NOT EXISTS sortie_type (
...
CREATE TABLE IF NOT EXISTS statut_armp (
    stt_armp_code SERIAL PRIMARY KEY,
    stt_armp_statut VARCHAR(50)
);

CREATE TABLE IF NOT EXISTS rang_hierarchique (
    rhq_code SERIAL PRIMARY KEY,
    rhq_rang VARCHAR(50),
    rhq_niveau VARCHAR(10)
);

CREATE TABLE IF NOT EXISTS tache_suppl (
...
CREATE TABLE IF NOT EXISTS motif_affectation (
...
CREATE TABLE IF NOT EXISTS objet_remboursement (
...
CREATE TABLE IF NOT EXISTS occupation_poste (
...
CREATE TABLE IF NOT EXISTS employe (
    ...
    emp_cin VARCHAR(100),
    emp_disponibilite BOOLEAN DEFAULT TRUE,
    date_entree DATE,
    date_sortie DATE,
...
CREATE TABLE IF NOT EXISTS pos_emp (
...
CREATE TABLE IF NOT EXISTS statut_emp (
    emp_code INTEGER REFERENCES employe(emp_code),
...
CREATE TABLE IF NOT EXISTS comp_employe (
    emp_code INTEGER REFERENCES employe(emp_code),
...
CREATE TABLE IF NOT EXISTS comp_poste (
...
CREATE TABLE IF NOT EXISTS doc_emp (
...
CREATE TABLE IF NOT EXISTS eval_stage (
...
CREATE TABLE IF NOT EXISTS stage_carriere (
    emp_code INTEGER REFERENCES employe(emp_code),
    pst_code INTEGER REFERENCES poste(pst_code),
    stg_code INTEGER REFERENCES stage(stg_code),
...
CREATE TABLE IF NOT EXISTS conj_status (
...
CREATE TABLE IF NOT EXISTS debit_solde_cng (
    emp_code INTEGER REFERENCES employe(emp_code),
...
CREATE TABLE IF NOT EXISTS debit_solde_prm (
    emp_code INTEGER REFERENCES employe(emp_code),
...
CREATE TABLE IF NOT EXISTS validation_cng (
    cng_code INTEGER REFERENCES conge(cng_code),
...
CREATE TABLE IF NOT EXISTS validation_prm (
    prm_code INTEGER REFERENCES permission(prm_code),
    sign_code INTEGER REFERENCES signature(sign_code),
    val_code SERIAL UNIQUE,
    val_date DATE,
...
CREATE INDEX IF NOT EXISTS idx_employe_matricule ON employe(emp_matricule);
CREATE INDEX IF NOT EXISTS idx_employe_im_armp ON employe(emp_im_armp);
CREATE INDEX IF NOT EXISTS idx_employe_nom_prenom ON employe(emp_nom, emp_prenom);
CREATE INDEX IF NOT EXISTS idx_affectation_emp ON affectation(emp_code);
CREATE INDEX IF NOT EXISTS idx_affectation_poste ON affectation(pst_code);
CREATE INDEX IF NOT EXISTS idx_affectation_dates ON affectation(affec_date_debut, affec_date_fin);
CREATE INDEX IF NOT EXISTS idx_conge_emp ON conge(emp_code);
CREATE INDEX IF NOT EXISTS idx_conge_dates ON conge(cng_debut, cng_fin);
...
CREATE INDEX IF NOT EXISTS idx_stage_dates ON stage(stg_date_debut, stg_date_fin);
CREATE INDEX IF NOT EXISTS idx_stagiaire_contact ON stagiaire(stgr_contact);
...
COMMENT ON TABLE employe IS 'Table principale unifiée des employés (source: carrière-stagiaire)';
...
SQL;
    ...
    {
        // Ordre inversé pour respecter les FK
        $tables = [
            'signature_engagement', 'signature_demande', 'piece', 'demande_remb',
            'pris_en_charge', 'engagement', 'etat_remb', 'facture', 'centre_sante',
            'validation_prm', 'validation_cng', 'signature',
            'interim_permission', 'debit_solde_prm', 'solde_permission', 'permission',
            'interim_conge', 'debit_solde_cng', 'solde_conge', 'interruption', 'conge',
            'enfant', 'emp_conj', 'conjointe', 'conj_status',
            'stage_carriere', 'doc_stage', 'stage', 'eval_stage', 'assiduite', 'stagiaire',
            'doc_emp', 'comp_poste', 'comp_employe', 'competence',
            'sortie', 'affectation', 'statut_emp', 'contact', 'pos_emp', 'employe',
            'occupation_poste', 'poste', 'service', 'direction',
            'objet_remboursement', 'type_centre', 'decision', 'type_conge', 'region',
            'etablissement', 'motif_affectation', 'position_', 'type_document',
            'tache_suppl', 'rang_hierarchique', 'statut_armp', 'type_contrat',
            'sortie_type', 'type_entree', 'modification', 'users'
        ];

        $this->db->query("DROP VIEW IF EXISTS v_etat_conge CASCADE");
